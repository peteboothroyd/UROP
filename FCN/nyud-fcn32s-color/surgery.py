...
def transplant(new_net, net, suffix=''):
    ...
    for p in net.params:
        ...
        if p_new not in new_net.params:
            print('dropping {0}'.format(p))
            continue
        for i in range(len(net.params[p])):
            if i > (len(new_net.params[p_new]) - 1):
                print('dropping {0} {1}'.format(p, i))
                break
            if net.params[p][i].data.shape != new_net.params[p_new][i].data.shape:
                print('coercing {0} {1} from {2} to {3}'.format(p, i, net.params[p][i].data.shape, new_net.params[p_new][i].data.shape))
            else:
                print('copying {0} -> {1} {2}'.format(p, p_new, i))
